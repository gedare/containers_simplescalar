	     ...
	     int zero_bss_segs)		/* zero uninit data segment? */
{
  ...
  /* EL read the symbol table STRINGTABLEID is the index in this particular file !!*/
 	if (fseek(fobj, fhdr.e_shoff + (STRINGTABLEID * fhdr.e_shentsize), SEEK_SET) < 0)
    ...
    /* allocate space for string table */
	  byte_t* strsymtab;
	  strsymtab= (char *)calloc(shdr.sh_size, sizeof(char));
	  if (!strsymtab)
	    fatal("out of virtual memory");
	  /* read the string table */
	  if (fseek(fobj, shdr.sh_offset, SEEK_SET) < 0)
	    fatal("cannot seek to string table section");
	  if (fread(strsymtab, shdr.sh_size, 1, fobj) < 0)
	    fatal("cannot read string table section");
  ...
  for (i=0; i < fhdr.e_shnum; i++)
	...
	fatal("file is dynamically linked, compile with `-static'");
	  ...
	  /*EL*/ if(shdr.sh_type == SHT_SYMTAB)
	  	{
	  		container_initialize();
	  		debug("EL Found Symbol table, loading in the component mem `%s'...", &strtab[shdr.sh_name]);
			if (fseek(fobj, shdr.sh_offset, SEEK_SET) < 0)
			...
			elf_sym* lastSymbol = (elf_sym*)(((char*)buffer)+shdr.sh_size) ;
			elf_sym* iterate = (elf_sym*) buffer;
			for(;iterate<lastSymbol;iterate++){
				if(ELF32_ST_TYPE((*iterate).st_info) == STT_FUNC && ELF32_ST_BIND((*iterate).st_info) == STB_GLOBAL)
				{
					container_add((*iterate).st_value,&strsymtab[(*iterate).st_name]);
					debug("EL Symbol `%s' %x...",&strsymtab[(*iterate).st_name] , (*iterate).st_value);
				}
	  		}
			if (buffer != NULL)
			{
