...
ruu_dispatch(void)
{
  ...
  while (/* instruction decode B/W left? */
      ...
      if (!in_flow)
	{
	  ...
	  if (!spec_mode){
  	  /*EL*/ /*at this point the program counter is correct with the actual value of the instruction being executed*/
	  /*EL */ //printf("pc = %x npc=%x sim_total_insn=%d sim_num_insn=%d op=%d \n",regs.regs_PC,regs.regs_NPC,sim_total_insn,sim_num_insn, op); 
	  /*EL */ container_traceFunctioncall(regs.regs_PC);
	    sim_num_insn++;
	  	}
...
#include "machine.def"
  ...
  /*EL */ //printf("The first call is to %x \n",regs.regs_PC);
  /*EL */ //container_traceFunctioncall(regs.regs_PC); 
