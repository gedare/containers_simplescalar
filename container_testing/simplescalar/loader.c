	     ...
	     int zero_bss_segs)		/* zero uninit data segment? */
{
  ...
  /* EL read the symbol table  24 is the index in this particular file !!*/
 	if (fseek(fobj, fhdr.e_shoff + (24 * fhdr.e_shentsize), SEEK_SET) < 0)
	  ...
	  /*EL*/ if(shdr.sh_type == SHT_SYMTAB)
	  	{
			...
			componentTableSize = shdr.sh_size;
			
			symbols = (elf_sym*) buffer;
